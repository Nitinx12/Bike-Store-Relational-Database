...
def _fk_check(
    ...
    """
    return gxe.UnexpectedRowsExpectation(unexpected_rows_query=query, description=description)
...
def customers_suite() -> list:
    return [
        ...
        *_not_null(
            ...
            "street", "city", "state", "zip_code", "updated_at",
        ),
        ...
        gxe.ExpectColumnValuesToBeBetween(column="zip_code", min_value=1, max_value=99999),
    ]
...
def staffs_suite() -> list:
    return [
        ...
        *_not_null("staff_id", "first_name", "last_name", "email", "active", "store_id", "updated_at"),
        gxe.ExpectColumnValuesToMatchRegex(column="email", regex=EMAIL_REGEX),
        gxe.ExpectColumnValuesToMatchRegex(column="phone", regex=PHONE_REGEX),
        ...
        _fk_check(
            ...
            "manager_id", "staffs", "staff_id", nullable=True,
        ),
    ]
...
def products_suite() -> list:
    return [
        ...
        *_not_null("product_id", "product_name", "brand_id", "category_id", "model_year", "list_price", "updated_at"),
        gxe.ExpectColumnValuesToBeBetween(column="list_price", min_value=0, strict_min=True),
        ...
        gxe.ExpectColumnValuesToBeBetween(column="model_year", min_value=2000, max_value=2027),
        _fk_check(
...
def stocks_suite() -> list:
    return [
        gxe.ExpectColumnValuesToBeUnique(column="store_id", mostly=0),  # placeholder, replaced below
    ][:0] + [  # keep structure simple: build the real list explicitly
        gxe.ExpectTableRowCountToBeBetween(min_value=1),
...
def orders_suite() -> list:
    return [
        ...
        *_not_null("order_id", "customer_id", "order_status", "order_date", "required_date", "store_id", "staff_id", "updated_at"),
        ...
        gxe.ExpectColumnValueLengthsToBeBetween(column="order_status", min_value=1, max_value=50),
        gxe.ExpectColumnPairValuesAToBeGreaterThanB(
            column_A="required_date", column_B="order_date", or_equal=True,
            ignore_row_if="either_value_is_missing",
        ),
        gxe.ExpectColumnPairValuesAToBeGreaterThanB(
            column_A="shipped_date", column_B="order_date", or_equal=True,
            ignore_row_if="either_value_is_missing",
        ),
        _fk_check(
...
def order_items_suite() -> list:
    ...
    return [
        ...
        *_not_null("order_id", "item_id", "product_id", "quantity", "list_price", "discount", "total_value", "updated_at"),
