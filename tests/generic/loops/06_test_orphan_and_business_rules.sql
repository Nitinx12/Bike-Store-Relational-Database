...
DECLARE
    v_checks text[][] := ARRAY[
        ...
        ARRAY['business_shipped_order_missing_or_future_date', $q$SELECT o.* FROM public.orders o WHERE o.order_status IN ('Completed') AND (o.shipped_date IS NULL OR o.shipped_date::date > CURRENT_DATE)$q$],
        ARRAY['business_unshipped_order_has_shipped_date', $q$SELECT o.* FROM public.orders o WHERE o.order_status IN ('Pending', 'Processing', 'Rejected') AND o.shipped_date IS NOT NULL$q$],
